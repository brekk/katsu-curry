...
// TODO: implement this
// const standardPlaceholderTest = (placeholder, fn) => {
//   const curried = fn(_divide)
//   const identity = curried(placeholder, 1)
//   const X = Math.round(Math.random() * 100) + 1
//   return identity(X) === random
// }

const qualifier = CURRY(([placeholderTest, placeholder], name, competitor) => {
  /* eslint-disable fp/no-throw */
  const divide = competitor(_divide)
  if (typeof divide(1) !== `function`) {
    throw new Error(`${name} cannot curry a single parameter.`)
  }
  if (divide(1)(2) !== 0.5) {
    throw new Error(`${name} didn't result in a number.`)
  }
  ...
  if (typeof placeholderTest === `function`) {
    if (!placeholderTest(placeholder, competitor)) {
      throw new Error(`${name} claimed to have a placeholder ability which does not work.`)
    }
  }
  /* eslint-enable fp/no-throw */
})
const noPlaceholderTest = qualifier([])
...
const addRunner = CURRY((runner, inner, testFunctionObject, suite) => {
  ...
  const addTestFunction = (importedFn, name) => {
    noPlaceholderTest(name, importedFn) // side effect: throws
    const cleanName = padify(name)
    const curriedFn = importedFn(inner)
    const runnable = runner(curriedFn)
    // run a qualifier on these fools one time
